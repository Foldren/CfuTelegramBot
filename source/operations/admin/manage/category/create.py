...
from components.extends import get_callb_content, get_msg_queue, generate_zero_array, \
...
async def save_add_category(callback: CallbackQuery, state: FSMContext):
    ...
    for i in range(0, len(data_category['user'])):
        if data_category['status_list'][i] == 1:
            list_id_users.append(int(data_category['user'][i]['chat_id']))
